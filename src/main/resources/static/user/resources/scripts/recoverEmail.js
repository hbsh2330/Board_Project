    ...
    xhr.onreadystatechange = function() {
        ...
        switch (responseObject['result']) {
            case 'failure':
                dialog.show({
                    title: '경고',
                    content: '입력한 정보와 일치하는 회원을 찾을 수 없습니다.<br><br>실명과 연락처를 올바르게 입력하였는지 다시 확인해 주세요.',
                    buttons: [
                        dialog.createButton('확인', function() {
                            dialog.hide();
                            recoverEmailForm['name'].focus();
            ...
            case 'success':
                dialog.show({
                    title: '이메일 찾기',
                    content: `입력한 정보로 찾은 이메일은 <b>${responseObject['email']}</b>입니다.<br><br>아래 <b>로그인하러 가기</b>버튼을 클릭하면 로그인 페이지로 이동합니다.`,
                    buttons: [
